...
class PandaPushEnv(PandaGymEnvironment):
    ...
    def __init__(self,
                 ...
                 search_space_id: int=0,  # 0
                 qacc_factor: float=0.3,
                 norm_reward=False,
                 command_type="acc",
        ...
        self.default_task = self.get_default_task()
        self.max_train_step = 4000000
        self.least_train_step = 2000000
        self.dynamics_group = [(0,),(1,2,),(3,),(4,5,)]
    ...
    def get_random_task(self):
        ...
        # Build per-parameter sequences, falling back to linear spacing when logs undefined
        series_task = np.zeros((5, bound.shape[0]))
        for idx, (low, high) in enumerate(bound):
            if low == high:
                series_task[:, idx] = low
                continue
            # Geometric spacing only valid when interval stays strictly away from zero and keeps sign
            same_sign = (low > 0 and high > 0) or (low < 0 and high < 0)
            if same_sign:
                if low > 0:
                    series_task[:, idx] = np.exp(np.linspace(np.log(low), np.log(high), 5))
                else:
                    # both negative, mirror via absolute values then reapply sign
                    geom = np.exp(np.linspace(np.log(-low), np.log(-high), 5))
                    series_task[:, idx] = -geom
            else:
                series_task[:, idx] = np.linspace(low, high, 5)
    ...
    def get_all_task_lower_bound(self):
        """Vectorized lower bounds ordered by current dyn_type."""
        indices = sorted(self.dyn_ind_to_name.keys())
        return np.array([self.get_task_lower_bound(idx) for idx in indices])

    def get_all_task_upper_bound(self):
        """Vectorized upper bounds ordered by current dyn_type."""
        indices = sorted(self.dyn_ind_to_name.keys())
        return np.array([self.get_task_upper_bound(idx) for idx in indices])


    def get_default_task(self):
        """Returns the default dynamics parameter values as a task vector"""
        self._save_init_dynamics_values()
        default_values = {
