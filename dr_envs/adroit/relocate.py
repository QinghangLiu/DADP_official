...
class RelocateEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, xml_file: str = 'relocate', **kwargs):
        self.target_obj_sid = 0
        self.S_grasp_sid = 0
        self.obj_bid = 0
        curr_dir = os.path.dirname(os.path.abspath(__file__))
        mujoco_env.MujocoEnv.__init__(self, curr_dir + f'/assets/{xml_file}.xml', 5, **kwargs)
    ...
    def step(self, a):
        ...
        if ADD_BONUS_REWARDS:
            if np.linalg.norm(obj_pos-target_pos) < 0.1:
                reward += 20.0                                        # bonus for object close to target
    ...
    def reset_model(self):
        ...
        self.sim.forward()
        #reset if achieved goal in initial state or object 
        return self.get_obs()
    ...
    def mj_viewer_setup(self):
        if self.render_mode == 'rgb_array':
            ...
            if not hasattr(self, 'viewer') or self.viewer is None or isinstance(self.viewer, MjViewer):
