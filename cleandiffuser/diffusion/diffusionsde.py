...
class ContinuousDiffusionSDE(BaseDiffusionSDE):
    ...
    def add_noise(self, x0, t=None, eps=None,**kwargs):
    ...
    def add_full_noise(self, x0, **kwargs):
    ...
    def sample(
        ...
        - **kwargs:
            Important: Task id or task embedding is required here.
        ...
        if isinstance(warm_start_reference, torch.Tensor) and warm_start_forward_level > 0.:
            ...
            xt = self.add_full_noise(prior,**kwargs) * temperature
...
class EnvFactorGuidedDiffusionSDE(ContinuousDiffusionSDE):
    """Continuous-time Diffusion SDE (VP-SDE)
    ...
    def __init__(
        ...
        if task is not None:
            ...
            raise ValueError("task is required for EnvFactorGuidedDiffusionSDE.")
    ...
    def loss(self, x0, condition=None, **kwargs):
        if kwargs.get("task_id", None) is not None:
            ...
            raise ValueError("task_id is required for loss calculation.")
        condition = self.model["condition"](condition) if condition is not None else None
    ...
    def add_full_noise(self, x0,**kwargs):
        if kwargs.get("task_id", None) is None:
            raise ValueError("task_id is required for add_full_noise.")
        xt = (torch.randn_like(x0) + self.guide_noise[kwargs['task_id'].squeeze(-1).int(), ...])/np.sqrt(1 + self.guide_noise_scale)
        return xt 
    
class EmbeddingGuidedDiffusionSDE(ContinuousDiffusionSDE):
    """Continuous-time Diffusion SDE (VP-SDE)
    ...
    def add_noise(self, x0, t=None, eps=None,embedding = None):

        t = (torch.rand((x0.shape[0],), device=self.device) *
             (self.t_diffusion[1] - self.t_diffusion[0]) + self.t_diffusion[0]) if t is None else t

        eps = (torch.randn_like(x0) + self.guide_noise_scale * embedding)  / np.sqrt(1 + self.guide_noise_scale) if eps is None else eps
        alpha, sigma = self.noise_schedule_funcs["forward"](t, **(self.noise_schedule_params or {}))
        alpha = at_least_ndim(alpha, x0.dim())
        sigma = at_least_ndim(sigma, x0.dim())
    ...
    def add_full_noise(self, x0,**kwargs):
        if kwargs.get("embedding", None) is None:
            raise ValueError("embedding is required for add_full_noise.")
        else:
            xt = (torch.randn_like(x0) + self.guide_noise_scale * kwargs['embedding']) / np.sqrt(1 + self.guide_noise_scale)
            # xt = (torch.randn_like(x0) + self.guide_noise_scale * kwargs['embedding']) 
        return xt  

    def loss(self, x0, condition=None, **kwargs):
        if kwargs.get("embedding", None) is not None:
            embedding = kwargs["embedding"]
        
            xt, t, eps = self.add_noise(x0, embedding=embedding)
        else:
            raise ValueError("embedding is required for loss calculation.")
        condition = self.model["condition"](condition) if condition is not None else None
...
class MixedDDIM(EmbeddingGuidedDiffusionSDE):
    """Continuous-time Diffusion SDE (VP-SDE)
    ...
    def __init__(
        ...
        self.guide_noise_scale = guide_noise_scale
        self.predict_embedding = bool(predict_embedding)
    # ==================== Training: Score Matching ======================
    ...
    def add_noise(self, x0, t=None, eps=None,embedding = None):
        ...
        xt = alpha * (x0 - embedding*self.guide_noise_scale) + embedding*self.guide_noise_scale + sigma * eps
        xt = (1. - self.fix_mask) * xt + self.fix_mask * x0
        if self.predict_embedding:
            eps = eps * sigma + (1-alpha)*embedding*self.guide_noise_scale
        return xt, t, eps
    ...
    def sample(
        ...
        for i in reversed(loop_steps):
            ...
            if solver == "ddpm":
                ...
                if self.predict_embedding:
                    xt = (alphas[i - 1] * ((xt - eps_theta) / alphas[i]) + sigmas[i - 1] * eps_theta / sigmas[i])
                else:
                    xt = (alphas[i - 1] * ((xt - self.guide_noise_scale * embedding - sigmas[i] * eps_theta) / alphas[i]) + sigmas[i - 1] * eps_theta) + self.guide_noise_scale * embedding * (alphas[i-1] + (sigmas[i-1]/sigmas[i]) * (1-alphas[i]))
            elif solver == "ode_dpmsolver_1":
                xt = (alphas[i - 1] / alphas[i]) * xt - sigmas[i - 1] * torch.expm1(hs[i]) * eps_theta
